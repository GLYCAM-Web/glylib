 ...
 */
double *
create_rotation_matrix(coord point, vector direction, double theta)
{
...
void
apply_rotation_matrix(coord *c, double *matrix)
{
    coord temp = *c;
...
void 
rotate_coords(coord **coords, int num_coords, coord point, 
              vector direction, double theta)
{
    ...
    for(i = 0; i < num_coords; i++)
        apply_rotation_matrix(coords[i], matrix);

    destroy_rotation_matrix(matrix);
}

coord
find_attachment_point(coord a, coord b, coord c,
                      double theta, double phi, double distance)
{
    vector lmn_x, lmn_y, lmn_z;
    double x_p, y_p, z_p;

    vector cb = get_vector(c, b);
    vector ba = get_vector(b, a);

    lmn_y = normalize(cross(ba, cb));
    lmn_z = normalize(cb);
    lmn_x = cross(lmn_y, lmn_z);

    x_p = distance * sin(theta) * cos(phi);
    y_p = distance * sin(theta) * sin(phi);
    z_p = distance * cos(theta);

    return (coord) {lmn_x.i*x_p + lmn_y.i*y_p + lmn_z.i*z_p + c.i,
